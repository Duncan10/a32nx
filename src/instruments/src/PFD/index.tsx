...
const AltModeSelected = () => {
    if (getSimVar('AUTOPILOT ALTITUDE SLOT INDEX', 'number') === 2 && getSimVar('L:AP_CURRENT_TARGET_ALTITUDE_IS_CONSTRAINT', 'number') !== 0) {
        return false;
    }
    return true;
};
...
const PFD = () => {
    ...
    let targetAlt = getSimVar('AUTOPILOT ALTITUDE LOCK VAR:3', 'feet');
    let isManaged = false;
    if (!AltModeSelected()) {
    ...
    const [targetMach] = useSimVar('AUTOPILOT MACH HOLD VAR', 'number');
    let targetSpeedCorrected = targetSpeed;
    if (isMach) {
        targetSpeedCorrected = SimVar.GetGameVarValue('FROM MACH TO KIAS', 'number', targetMach);
    }
    ...
    return (
        <svg className="pfd-svg" version="1.1" viewBox="0 0 158.75 158.75" xmlns="http://www.w3.org/2000/svg">
            ...
            <AirspeedIndicatorOfftape airspeed={clampedAirspeed} airspeedAcc={filteredAirspeedAcc} targetSpeed={targetSpeedCorrected} speedIsManaged={!isSelected} />
